...
class DataPreprocess:
    ...
    def make_embedding(self):
        ...
        for i, word in enumerate(self.embedding.wv.key_to_index.items()):
            word = word[0] #因为word默认是元组
            # print('get word #{} {}'.format(i+1, word), end='\r')
            self.word2index[word] = len(self.word2index)
            self.index2word.append(word)
            # print(self.embedding.wv[word].shape)
